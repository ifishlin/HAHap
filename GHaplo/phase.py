...
from GHaplo.vcf import output_phasing2VCF, split_vcf_by_chrom, output_cc2csv
...
from GHaplo.assembly import HiMergeNode, hc_merge, save_phasing
...
def pipeline(args, chrom, connected_components, ref_alt_list, genomic_location_list, timer, hc_type):
    ...
    for cc_idx, cc in enumerate(connected_components):
        ...
        timer.start('prepare')
        for c in cc_sorted:
            phase_input.append([chrom, int(genomic_location_list[c]), 0, 'snp', ref_alt_list[c][0],
                                ref_alt_list[c][0], ref_alt_list[c][1]])
