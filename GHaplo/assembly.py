...
def get_maxscore_between_segments(group1, group2, mut_info, rank):
    ...
    sorted_x = sorted(rank_dict.items(), key=operator.itemgetter(1, 0), reverse=True)
...
def choice_haplos(observed_pairs_dict, cluster1, cluster2, idx_in_c1, idx_in_c2, sv_dict, phase_variants_location_list,
                  phase_variants_called_list, encoding_tb, heter, hratio, pv_dict, variant_num):
    """
    ...
    h1, h2 = hc_merge_haplo(sorted_x, c1_anchor, c2_anchor, idx_in_c1, idx_in_c2, cluster1, cluster2, sv_dict,
                            phase_variants_location_list, phase_variants_called_list, encoding_tb, heter, hratio,
                            pv_dict, variant_num)

    return h1, h2


def hc_merge_haplo(sorted_x, c1_anchor, c2_anchor, idx_in_c1, idx_in_c2, cluster1, cluster2, sv_dict,
                   phase_variants_location_list, reference, encoding_tb, heter, hratio, pv_dict, variant_num):
    """
    """
    ...
    if c1_anchor is None and c2_anchor is None:
        ...
        if seam_point_count > b_threshold and local_brute: 
            ...
            sol1_h1, sol1_h2, sol2_h1, sol2_h2, sorted_name, t_sol1_h1, t_sol1_h2, t_sol2_h1, t_sol2_h2 \
                = build_two_possible_sol(cluster1, cluster2)
...
def hc_merge(vars_cddt_mp, vars_cddt_pl, pv_dict, score_matrix, sv_dict, phase_variants_location_list,
             phase_variants_called_list, encoding_tb, heter, hratio, timer):
    """
    Args:
    Return:
    """
    ...
    while t != -1 and u != -1:    
        ...
        for rank in range(0, v1c * v2c):
            ...
            if pv_key in pv_dict:
                ...
                node.h1, node.h2 = choice_haplos(observed_pairs_dict, c1, c2, c1_idx, c2_idx, sv_dict,
                                                 phase_variants_location_list, phase_variants_called_list, encoding_tb,
                                                 heter, hratio, pv_dict, v_num)
